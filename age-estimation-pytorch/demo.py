...
def yield_images_from_dir(img_dir):
    ...
    img_paths = [os.path.join(img_dir, x) for x in sorted(os.listdir(img_dir)) if x.endswith(".jpg") and x[0] != "."]
    ...
    for img_path in img_paths:
        ...
        if img is not None:
            ...
            yield cv2.resize(img, (int(w * r), int(h * r))), img_path
...
def main():
    ...
    img_generator_len = len(os.listdir(img_dir))
    output_arr = []
    pbar = tqdm(image_generator, total=img_generator_len)

    with torch.no_grad():
        for idx, (img, name) in enumerate(pbar):
