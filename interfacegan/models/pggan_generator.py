...
class PGGANGenerator(BaseGenerator):
    ...
    def build(self):
        ...
        self.model = PGGANGeneratorModel(resolution=self.resolution, fused_scale=self.fused_scale,
    ...
    def synthesize(self, latent_codes):
        ...
        if not (len(latent_codes_shape) == 2 and latent_codes_shape[0] <= self.batch_size and latent_codes_shape[1] == self.latent_space_dim):
