...
from models.stylegan.Blocks import (DiscriminatorBlock, DiscriminatorTop,
                           GSynthesisBlock, InputBlock)
from models.stylegan.CustomLayers import (EqualizedConv2d, EqualizedLinear,
...
class GSynthesis(nn.Module):
    ...
    def __init__(self, dlatent_size=512, num_channels=3, resolution=1024,
        ...
        # :param use_noise: True = randomize noise inputs every time (non-deterministic),
...
class Generator(nn.Module):
    ...
    def forward(self, latents_in, depth, alpha, labels_in=None, input_is_latent=False, return_latents=False):
        """
        ...
        if return_latents:
            ...
            return fake_images, None
    
    def mean_latent(self, n_latent, device):
        latents_in = torch.randn(
            n_latent, self.latent_size, device=device
        )
        latent = self.g_mapping(latents_in).mean(0, keepdim=True)
