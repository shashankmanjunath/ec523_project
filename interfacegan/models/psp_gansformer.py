...
class pSpGansformer(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.latent_dim = 512
        self.latent_space_dim = self.latent_dim
        self.batch_size = 1
    ...
    def sample(self, num, latent_space_type='Z'):
        ...
        if latent_space_type == 'Z':
            latent_codes = torch.randn([num, 17, 15, 32], device=torch.device("cpu")) / 10
    ...
    def synthesize(self, latent_codes, latent_space_type='Z', generate_style=False, generate_image=True):
        ...
        if generate_image:
            images = self.decoder(ws=torch.as_tensor(ws).to(self.run_device))[0]
