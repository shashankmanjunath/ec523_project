...
from models.psp_generator import PSPGenerator
...
def main():
    ...
    elif gan_type == "psp_stylegan2":
        opts = convert_to_opts(args)
        model = PSPGenerator(args.model_name, opts, logger)
        kwargs = {'latent_space_type': args.latent_space_type}
    #  elif gan_type == "psp_gansformer":
    #      opts = convert_to_opts(args)
    #      model = pSpGansformer(opts)
    #      kwargs = {'latent_space_type': args.latent_space_type}
    else:
    ...
    for latent_codes_batch in model.get_batch_inputs(latent_codes):
        if gan_type == 'pggan':
            ...
            outputs = model.easy_synthesize(latent_codes_batch, **kwargs, generate_style=args.generate_style,
                    generate_image=args.generate_image)
        elif gan_type == "psp_stylegan2":
            outputs = model.easy_synthesize(latent_codes_batch, **kwargs, generate_style=args.generate_style,
                    generate_image=args.generate_image)
        if gan_type == 'psp_gansformer':
            outputs = model.easy_synthesize(latent_codes_batch)
